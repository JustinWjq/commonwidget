...
import com.common.widget.dialog.enums.LayoutStatus;
...
public class SmartDragLayout extends FrameLayout implements NestedScrollingParent {
    ...
    private void finishScroll() {
        if (enableDrag) {
            ...
            scroller.startScroll(getScrollX(), getScrollY(), 0, dy, TxPopup.getAnimationDuration());
    ...
    public void smoothScroll(final int dy, final boolean isOpen) {
        post(new Runnable() {
            @Override
            public void run() {
                scroller.startScroll(getScrollX(), getScrollY(), 0, dy, (int)(isOpen ? TxPopup.getAnimationDuration() : TxPopup.getAnimationDuration()*0.8f));
