...
class FitsKeyboard implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    FitsKeyboard(TxImmersionBar immersionBar) {
    ...
    public void onGlobalLayout() {
        if (mImmersionBar != null && mImmersionBar.getBarParams() != null && mImmersionBar.getBarParams().keyboardEnable) {
            TxBarConfig barConfig = mImmersionBar.getBarConfig();
            ...
            if (keyboardHeight != mTempKeyboardHeight) {
                ...
                if (!TxImmersionBar.checkFitsSystemWindows(mWindow.getDecorView().findViewById(android.R.id.content))) {
                    if (mChildView != null) {
                ...
                if (!isPopup && mImmersionBar.getBarParams().barHide != TxBarHide.FLAG_SHOW_BAR) {
