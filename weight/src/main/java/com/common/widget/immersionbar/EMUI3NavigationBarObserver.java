...
final class EMUI3NavigationBarObserver extends ContentObserver {
    ...
    private ArrayList<TxImmersionCallback> mCallbacks;
    ...
    public void onChange(boolean selfChange) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1 && mApplication != null && mApplication.getContentResolver() != null
            ...
            for (TxImmersionCallback callback : mCallbacks) {
                callback.onNavigationBarChange(show != 1);
            }
        }
    }

    void addOnNavigationBarListener(TxImmersionCallback callback) {
        if (callback == null) {
            return;
        }
        if (mCallbacks == null) {
    ...
    void removeOnNavigationBarListener(TxImmersionCallback callback) {
